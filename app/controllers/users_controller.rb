class UsersController < ApplicationController
  ...
  def toggle_advanced_search
    ...
    if @user.advanced_search == "disabled"
      ...
      if @user.butterfly_number.to_i >= @adv_search_butterflies
        ...
        if @user.save
          redirect_to privacy_settings_path(butterfly: "animate"),
                      notice: "Advanced Search Enabled"
        else
          redirect_back fallback_location: root_path,
                        notice: "#{@user.errors.full_messages.first}"
        end
      ...
      if @user.save
        redirect_to privacy_settings_path,
                    notice: "Advanced Search Disabled"
      else
        redirect_back fallback_location: root_path,
                      notice: "#{@user.errors.full_messages.first}"
      end
  ...
  def next_unread_notification
    exclude_ids = params[:exclude_ids].to_s.split(',').map(&:to_i)
    @notification = current_user.notifications
      ...
      .first
    if @notification
      render partial: 'users/notification',
             locals: { notification: @notification, pinned: true },
             layout: false
    else
      render plain: ''
    end
