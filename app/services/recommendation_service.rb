class RecommendationService
  def self.with_preference(current_profile, max_level=5, matched_ids=[])
    ...
    filtered_profile_ids = [current_profile.id] +
      ...
      Friendship.where('(friendable_id = ? OR friend_id = ?) AND status = ?', current_profile.id, current_profile.id, 1).map { |f| f.friendable_id == current_profile.id ? f.friend_id : f.friendable_id }
