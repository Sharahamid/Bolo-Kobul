...
namespace :deploy do
  ...
  task :check_revision do
    on roles(:app) do
      remote_branch = "new-origin/#{fetch(:branch)}"
      unless `git rev-parse HEAD`.chomp == `git rev-parse #{remote_branch}`.chomp
        puts "WARNING: HEAD is not the same as #{remote_branch}"
